...
- **Scheduling.** GitHub Actions (`source-first-rebuild.yml`, self-hosted runner `knowledge-agent-sessions`) owns build/gate/promote. The only local LaunchAgent is `launchd/com.arjundivecha.pks-rebuild-kicker.plist` (cadence guard + freshness marker; never builds). The 23:00 `com.arjun.knowledge-ingestion` legacy `ke_` pipeline was **retired 2026-09-04** (no production consumer since the 08-09 source-first cutover); its plists are kept under `launchd/retired/`, its workflows remain `workflow_dispatch`-only as the rollback path.
