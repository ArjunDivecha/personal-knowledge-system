...
1. **Salience never collapsed — it was never discriminative.** `computeSalience` (`salience.ts:93-173`) is a pure function of a handful of *discrete* inputs: a 3-value confidence that is almost always `"medium"` (0.7), a mention-count boost that is a step function stuck at its first step for most entries, a decay term that is **exactly 1.0 for 4 of the 7 context types** (infinite half-lives, `memory_policy.json:22-25`), and a type-multiplier lookup. Two entries sharing {context type, confidence, mention bucket, same-day `last_seen`} produce **bit-identical** scores; the 4-decimal rounding then makes the ties exact. The 0.48–0.49 band is simply the arithmetic of `0.7 × decay × type_mult × freqBoost(2)`; the 1.0 pins are the `min(1.0, raw)` clamp (`salience.ts:162`), whose basin is wide — many structurally different entries saturate to the same ceiling. Worst of all, the one *continuous, meaningful* signal the formula defines — `retrievalBoost` from `last_accessed` — is **dead code in practice**: the `search` tool never writes `last_accessed` back on a hit (only merge helpers do, `index.ts:703`, `dream.ts:487`). Every entry I read has `access_count: 0`. The system is scoring memories while blind to whether they are ever used.
   ...
   > **⚠ CORRECTION (2026-07-09, verified against live production):** the "dead code" claim above is **wrong**. A full usage loop has existed since 2026-03-27: `scheduleReconsolidation`/`reconsolidateEntry` (`index.ts:1610-1727`) is wired into `search` (rank-1 result only), `get_context`, and `get_deep` — async (`ctx.waitUntil`), fail-open, durable side keys, salience recompute, vector PATCH, with a guard against un-archiving. The rank-1 cap is a deliberate 2026-06-09 decision (commit `34c590b`, "exposure ≠ use"): top-5 triggers were granting every search result archive immunity regardless of relevance. Verified empirically 2026-07-09: two live searches took the rank-1 hit from `access_count: 0 → 1`, salience `0.4786 → 0.6286` — exactly the `retrievalBoost` delta. The corpus-wide `access_count: 0` observation that misled this diagnosis is explained by **rank-1-only writes × low organic search volume**, not a missing mechanism. Consequences: the discrimination critique of salience stands (the signal is real but *sparse*); the stage-1 fix is NOT "build the loop" but "protect its semantics + stop eval-traffic pollution" — see the rewritten `contracts/usage-signal-loop.spec.md` (v2) and its ledger for the full account. Widening reinforcement beyond rank-1 would reintroduce the June bug and must not be done casually.
...
> **⚠ CORRECTION (2026-07-09):** Layer 1 as described already existed (see the §1.1.1 correction) — and its bound is deliberately *tighter* than proposed here: rank-1 only, not "≤ limit returned entries", per the 2026-06-09 exposure≠use decision. What shipped instead as stage 1: test-codification of the existing semantics plus a `suppress_access_signals` flag so nightly eval probes stop fabricating usage signal (`contracts/usage-signal-loop.spec.md` v2). Salience-v2's `usage` component (Layer 2 above) consumes the signal as-is; if rank-1-only proves too sparse for a 0.30-weight component, widen what counts as *genuine* use (get_context/get_deep already count; §5.3 session-utilization telemetry) — never candidate-pool exposure.
...
> **⚠ CORRECTION (2026-07-09):** premise falsified — the loop already existed (§1.1.1 correction). Stage 1 as executed: codify the existing rank-1/fail-open/minimal-write semantics in tests and exclude synthetic eval traffic from reinforcement (`suppress_access_signals`). The structural lesson is now policy for every remaining contract in this program: each gets a **G0 premise gate** — a cheap mechanical check that the defect the contract assumes actually exists — because the build-ladder escalates only on gate failure and cannot catch a wrong diagnosis whose gates pass green.
